...
Scenarios:
  ...
  Scenario: 2
    Given: At least one AWS DMS Endpoint exists
      And: SslMode is set to none
      Then: Return NON_COMPLIANT with annotation "This AWS DMS Endpoint has not enabled SSL"
  Scenario: 3
    Given: At least one AWS DMS Endpoint exists
      And: SslMode is set to require or verify-ca or verify-full.
...
def evaluate_compliance(event, configuration_item, valid_rule_parameters):
    ...
    for dms_endpoint_details in dms_endpoints:
        if dms_endpoint_details['SslMode']=='none':
        #SCENARIO 2
            evaluations.append(build_evaluation(dms_endpoint_details['EndpointIdentifier'], 'NON_COMPLIANT', event, annotation='This AWS DMS Endpoint does not have SSL enabled'))
        else:
            #SCENARIO 3
